...
Wildcard Pages allow a single page to respond to multiple paths. This feature is particularly useful for creating a generic response for certain types of requests.
...
Mikrob will attempt to find a direct match for the requested URL. If no direct match is found, it will then evaluate the URL against the regex defined in the `path` of wildcard pages.
